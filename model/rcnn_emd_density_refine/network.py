...
from module.density_rpn import density_fpn_roi_target
...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.DensityRPN = DensityRPN(config.rpn_channel)
        self.RCNN = RCNN()
    ...
    def _forward_train(self, image, im_info, gt_boxes):
        ...
        rpn_rois, density, loss_dict_rpn = self.DensityRPN(fpn_fms, im_info, gt_boxes) # proposals to pool, loss 
    ...
    def _forward_test(self, image, im_info):
        fpn_fms = self.FPN(image)
        rpn_rois, density = self.DensityRPN(fpn_fms, im_info)
